...
// Проверяем, есть ли товар в наличии хотя бы в одном размере
$totalStock = 0;
foreach ($product['sizes_stock'] as $qty) {
    $totalStock += $qty;
}
$isProductSoldOut = ($totalStock === 0);
?>
<!DOCTYPE html>
...
<head>
    ...
    <title><?php echo htmlspecialchars($product['name']); ?> | JaGGeR Shop</title>
...
<body class="min-h-screen flex flex-col bg-background text-foreground">
    ...
    <main class="flex-grow container py-12">
        <div class="product-page-layout">
            ...
            <!-- Левая колонка: Вертикальный Слайдер -->
            <div class="vertical-slider-container">
                <div class="vertical-slider-viewport">
                    <div class="vertical-slider-tape" id="sliderTape">
                        <?php foreach ($product['images'] as $imgUrl): ?>
                            <img src="<?php echo htmlspecialchars($imgUrl); ?>" alt="Фото">
                        <?php endforeach; ?>
                    </div>
                    
                    <!-- Стрелки управления в виде векторных SVG-шевкронов -->
                    <?php if (count($product['images']) > 1): ?>
                        <div class="vertical-slider-controls">
                            <button class="control-arrow" onclick="scrollVerticalSlider(-1)">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#ffffff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-chevron-up">
                                    <path d="m18 15-6-6-6 6"></path>
                                </svg>
                            </button>
                            <button class="control-arrow" onclick="scrollVerticalSlider(1)">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#ffffff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-chevron-down">
            ...
            <div class="product-page-details">
                <div class="product-title-row">
                    ...
                    <div class="price-row-detail">
                        <?php if ($product['old_price']): ?>
                            <span class="price-old-detail"><?php echo number_format($product['old_price'], 0, '', ' '); ?>&nbsp;₽</span>
                        <?php endif; ?>
                        <span class="price-current-detail"><?php echo number_format($product['price'], 0, '', ' '); ?>&nbsp;₽</span>
                    </div>
                </div>
                ...
                <div class="size-selector-wrap">
                    <p class="font-bold text-white">Выбрать размер:</p>
                    <div class="size-buttons" id="sizeSelector">
                        <?php 
                        $firstSize = true;
                        foreach ($product['sizes_stock'] as $sizeName => $stockQty): 
                        ?>
                            <button class="btn-size <?php echo $firstSize ? 'active' : ''; ?>" 
                ...
                <p class="stock-status" id="stockStatus">
                    <?php if ($isProductSoldOut): ?>
                        Нет в наличии
                    <?php else: ?>
                        В наличии: <span id="statusVal" class="text-orange">загрузка...</span>
                    <?php endif; ?>
                </p>

                <!-- ИНТЕРАКТИВНАЯ ГРУППА ДЕЙСТВИЯ КОРЗИНЫ -->
                <div class="product-cart-action-group" id="productCartActionGroup">
                    <?php if (!$isProductSoldOut): ?>
                        <!-- Состояние 1: Добавить в корзину (по умолчанию) -->
                        <button class="btn btn-primary w-full add-to-cart-page-btn" id="addToCartPageBtn" onclick="handlePageAddToCart()">
                            Добавить в корзину
                        </button>
                        
                        <!-- Состояние 2: Уже в корзине (кнопка убавления, прибавления и убрать) -->
                        <div class="added-state-container" id="addedStateContainer" style="display: none;">
                            <button class="btn btn-secondary in-cart-btn" onclick="handlePageRemoveFromCart()"></button>
                            <div class="quantity-selector">
                                <button class="qty-btn" onclick="adjustProductPageQty(-1)">-</button>
                                <span class="qty-val" id="productPageQtyVal">1</span>
                                <button class="qty-btn" onclick="adjustProductPageQty(1)">+</button>
                            </div>
                        </div>
                    <?php endif; ?>
    ...
    <script>
        const currentProductData = <?php echo json_encode($product); ?>;
    </script>
    <script>
